...
-- Create auth helper functions for RLS (using public schema instead of auth schema)
...
CREATE POLICY "Users can view their own profile" ON users
  FOR SELECT USING (public.auth_uid() = id);

CREATE POLICY "Users can update their own profile" ON users
  FOR UPDATE USING (public.auth_uid() = id);

CREATE POLICY "Users can insert their own profile" ON users
  FOR INSERT WITH CHECK (public.auth_uid() = id);
...
CREATE POLICY "Authenticated users can insert games" ON games
  FOR INSERT WITH CHECK (public.auth_role() = 'authenticated');

CREATE POLICY "Authenticated users can update games" ON games
  FOR UPDATE USING (public.auth_role() = 'authenticated');
...
CREATE POLICY "Users can view their own listings" ON listings
  FOR SELECT USING (public.auth_uid() = seller_id);

CREATE POLICY "Users can insert their own listings" ON listings
  FOR INSERT WITH CHECK (public.auth_uid() = seller_id);

CREATE POLICY "Users can update their own listings" ON listings
  FOR UPDATE USING (public.auth_uid() = seller_id);
...
CREATE POLICY "Authenticated users can insert bids" ON bids
  FOR INSERT WITH CHECK (public.auth_uid() = bidder_id);

-- Messages policies
CREATE POLICY "Users can view messages they sent or received" ON messages
  FOR SELECT USING (public.auth_uid() = sender_id OR public.auth_uid() = recipient_id);

CREATE POLICY "Users can insert messages" ON messages
  FOR INSERT WITH CHECK (public.auth_uid() = sender_id);
...
CREATE POLICY "Users can insert reviews" ON reviews
  FOR INSERT WITH CHECK (public.auth_uid() = reviewer_id);

-- Transactions policies
CREATE POLICY "Users can view their own transactions" ON transactions
  FOR SELECT USING (public.auth_uid() = buyer_id OR public.auth_uid() = seller_id);
...
CREATE POLICY "Users can manage their own collections" ON user_collections
  FOR ALL USING (public.auth_uid() = user_id);
...
CREATE POLICY "Users can view their own shipping labels" ON shipping_labels
  FOR SELECT USING (
    EXISTS (
      ...
      AND (transactions.buyer_id = public.auth_uid() OR transactions.seller_id = public.auth_uid())
    )
  );
